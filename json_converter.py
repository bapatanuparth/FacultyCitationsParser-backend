    ...
    for person in data:
        ...
        def append_items(items, df):
            ...
            for item in items:
                ...
                if item.get('elite_journal') not in [None, ""]:
...
def json_to_doc(data, output_path):
    ...
    for person in sorted_data:
        ...
        if person['publications']:
            ...
            for publication in person['publications']:
                doc.add_paragraph(publication['citation'], style='ListBullet')
        
        # Conferences
        if person['conferences']:
            doc.add_heading('Conferences:', level=2)
            for conference in person['conferences']:
                doc.add_paragraph(conference['citation'], style='ListBullet')
        
        # Book Chapters
        if person['bookChapters']:
            doc.add_heading('Book Chapters:', level=2)
            for chapter in person['bookChapters']:
                doc.add_paragraph(chapter['citation'], style='ListBullet')
